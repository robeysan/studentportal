...
function validate_field(obj) {
	...
	if($(obj).hasClass('validate')) {
		...
		for(key in classes){
			if(classes[key].indexOf('v-')==0 && localvalid != false){
				...
				switch(validator[0]){
					...
					break;
					case "requiredradio":		
						localvalid=validate_required_radio($(obj));
						$(obj).attr('title', 'Required');
					break;
					case "contains":
        ...
        // If it's a radio button we want to append after the container as
        // the radio buttons are inline and it looks weird
        if($(obj).attr('type') == 'radio') {
            obj_to_append_after = $(obj).parent();
        } else {
            obj_to_append_after = $(obj);
        }
		if(localvalid==false){
			$(obj).addClass('error');
			if($(obj).attr('title')!=''){
			...
			obj_to_append_after.parent().find('.label-important').remove();
			obj_to_append_after.parent().find('.label-success').remove();
			obj_to_append_after.after('<span class="label label-success">OK!</span>');
			return(true);
		}
	}
}

function validate_required_radio(radio_buttons){
