$(document).ready(function() {
	...
	// $('select').change(senddata);  // Causing extra writes in IE
	$('textarea').blur(senddata);

	//
	$('#birth_date').mask("99/99/9999");
	$('#SF_NO').mask("999-99-9999");
    ...
    // When Student Type changes, request
    // the program description and populate
    // dropdown
	$("#Student_type").change(function(){
		...
		$.ajax({
			async: false,
			type: "POST",
			data: "resource=prog_desc&data=" + $(this).val(),
	...
	$("#prog").change(function(){
		$.ajax({
			async: false,
			type: "POST",
			data: "resource=major&data=" + $(this).val() + "&interest=" + $("#prog_interest").val(),
			url: "app/get_resource",
			success: function(msg){
				$('#major').html(msg);
				$('#major').change();
	...
	$("#major").change(function(){
		$.ajax({
			async: false,
			type: "POST",
			data: "resource=primary_content&data=" + $(this).val(),
			url: "app/get_resource",
			success: function(msg){
				$('#subject1').html(msg);
				$('#subject1').change();
	...
	$("#subject1").change(function(){
		$.ajax({
			async: false,
			type: "POST",
			data: "resource=secondary_content&data=" + $(this).val(),
			url: "app/get_resource",
			success: function(msg){
				$('#subject2').html(msg);
				$('#subject2').change();
	...
	$("#college_name1").bind('blur', function() {
	...
	$("#college_name2").bind('blur', function() {
		$.get("app/get_ceeb", { name: $(this).val() }, function(data) {
	...
	$("#college_name3").bind('blur', function() {
		$.get("app/get_ceeb", { name: $(this).val() }, function(data) {
	...
	$("#college_name4").bind('blur', function() {
		$.get("app/get_ceeb", { name: $(this).val() }, function(data) {
	...
	$("#college_name5").bind('blur', function() {
		$.get("app/get_ceeb", { name: $(this).val() }, function(data) {
